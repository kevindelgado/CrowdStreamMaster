...
public class Operator implements Serializable, EndPoint, Connectable, Callback{
	...
	public synchronized void send(DataTuple dt) throws ValuedException {
		// We check the targets with our routers
		ArrayList<Integer> targets = router.forward(dt);
		try{
			processingUnit.sendData(dt, targets);
		} catch(ValuedException ex){//spica@110514
			int opId = ex.getValue();
			this.opContext.removeDownstream(opId);
			router.reconfigureRoutingImpl(this.getOpContext(), this.getOpContext().getOriginalDownstream());
			LOG.error("opId {} removed from router", opId);
		}
	}
