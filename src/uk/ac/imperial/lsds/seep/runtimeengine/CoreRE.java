...
public class CoreRE {
	...
	public void setRuntime(){
		...
		if(GLOBALS.valueFor("enableDistributedMonitor").equals("true")){
			//Monitor Thread
			monitorT = new Thread(){
				int interval = Integer.parseInt(GLOBALS.valueFor("monitorInterval"))*1000;
				@Override
				public void run(){
					while(ch.getGoOn()){
						coreProcessLogic.sendResourceInfoToUpstreams();
						try {
	 ...
	 * @throws ValuedException 
	 */
	public void startDataProcessing() throws ValuedException{
	...
	public void forwardData(DataTuple data) throws ValuedException{
	...
	public void processControlTuple(ControlTuple ct, OutputStream os, InetAddress remoteAddress) {
		...
		/** Processing Metric Info **/
		else if(ctt.equals(ControlTupleType.METRICS)){
			processMetrics(ct.getMetrics(), ct.getId());
		}
	}
