...
//		else System.out.println("IGNORED");
	...
	public synchronized void alertCPU(int opIdToParallelize) throws SocketException{
		System.out.println("#########################################################");
	...
	public synchronized boolean scaleOutOperator(int opIdToParallelize, int newOpId, Node newNode) throws SocketException{
		boolean result = false;
		
		try {
			if(GLOBALS.valueFor("checkpointMode").equals("light-state")){
				result = lightScaleOutOperator(opIdToParallelize, newOpId, newNode);
			}
	...
	public synchronized void scaleOutOperator1(){
		if(GLOBALS.valueFor("checkpointMode").equals("light-state")){
			lightScaleOutOperator1();
		}
	}
	
	public synchronized void scaleOutOperator2(int opIdToParallelize, int newOpId){
		if(GLOBALS.valueFor("checkpointMode").equals("light-state")){
			lightScaleOutOperator2(opIdToParallelize, newOpId);
		}
	}
	
	public boolean lightScaleOutOperator(int opIdToParallelize, int newOpId, Node newNode) throws ScaleOutException, SocketException{
...
//		inf.deployCodeToOperator(newOp);
		...
		//deploy new Operator
		//conn to new node
		boolean result = inf.remoteOperatorInstantiation(newOp);
	...
	public void lightScaleOutOperator1() throws KryoException{
		//add upstream conn
		//conn to down nodes
/**WAIT FOR ANSWER**/
	...
	public void lightScaleOutOperator2(int opIdToParallelize, int newOpId){
...
/**FINALIZE SCALE OUT PROTOCOL**/
	...
	public void largeScaleOutOperator(int opIdToParallelize, int newOpId, Node newNode) throws ScaleOutException, SocketException{
		//get number of upstreams to indicate msh how many messages to wait -> measurement purposes
		int numUpstreamsOpId = inf.getNumUpstreams(opIdToParallelize);
		//set initial time and number of downstreams
		Infrastructure.msh.setParallelizationStartTime(System.currentTimeMillis(), numUpstreamsOpId);
...
//	}
	...
	public void addDownstreamConnections(Operator newOp) throws KryoException{
